...
def send_welcome(message):
    ...
    users[user_id] = {"step": 0, "wallet": None, "twitter": None, "referrals": [], "subreferrals": []}
...
def send_step_message(user_id):
    ...
    if step == 0:
        ...
        markup.add(types.InlineKeyboardButton("✅ Done", callback_data="check_telegram"))
        bot.send_message(user_id, "✅ Step 1: Please join our Telegram Channel.", reply_markup=markup)
    elif step == 1:
        ...
        bot.send_message(user_id, "✅ Step 3: Please visit our website.", reply_markup=markup)
    elif step == 3:
...
@bot.callback_query_handler(func=lambda call: call.data == "check_telegram")
def check_telegram(call):
    ...
    try:
        ...
        if status in ["member", "creator", "administrator"]:
            ...
            bot.send_message(user_id, "✅ Telegram channel joined successfully.")
            send_step_message(user_id)
        else:
            bot.send_message(user_id, "❌ You must join the Telegram channel first.")
    except:
        bot.send_message(user_id, "⚠️ Could not verify your membership. Try again later.")
