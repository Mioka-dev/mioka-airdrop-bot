...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # NEW: Admin check on start command
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_username == ADMIN_USERNAME and query.data != 'get_admin_data':
        return

    if query.data.startswith('lang_'):
        lang = query.data.split('_')[1]
        set_user_language(user_id, lang)
        set_user_state(user_id, 'join_telegram')
        
        welcome_text = get_translation(lang, "welcome_message")
        keyboard = [
            [InlineKeyboardButton(get_translation(lang, "join_telegram_button"), url=LINKS['telegram_channel'])]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    elif query.data == 'done_twitter_follow': # NEW: Follow Done Button
        set_user_state(user_id, 'retweet_tweet')
    ...
    elif query.data == 'done_retweet': # NEW: Retweet Done Button
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state == 'join_telegram':
        # NEW: Show "Done" button after user sends a message. This assumes they've joined.
        set_user_state(user_id, 'done_telegram_check')
        keyboard = [[InlineKeyboardButton(get_translation(lang, "done_button"), callback_data='done_telegram')]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text(get_translation(lang, "after_join_telegram_message"), reply_markup=reply_markup)
        
    elif state == 'ask_telegram_username':
        users_data[user_id]['telegram_username'] = text
        set_user_state(user_id, 'follow_twitter')
    ...
    elif state == 'follow_twitter':
        # NEW: Show "Done" button for Twitter follow after user sends a message.
        set_user_state(user_id, 'done_twitter_follow_check')
        keyboard = [[InlineKeyboardButton(get_translation(lang, "done_button"), callback_data='done_twitter_follow')]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text(get_translation(lang, "after_follow_twitter_message"), reply_markup=reply_markup)

    elif state == 'retweet_tweet':
        # NEW: Show "Done" button for retweet after user sends a message.
        set_user_state(user_id, 'done_retweet_check')
        keyboard = [[InlineKeyboardButton(get_translation(lang, "done_button"), callback_data='done_retweet')]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text(get_translation(lang, "after_retweet_message"), reply_markup=reply_markup)

    elif state == 'ask_twitter_username':
        users_data[user_id]['twitter_username'] = text
        set_user_state(user_id, 'visit_website')
    ...
    elif state == 'visit_website':
        # NEW: Show "Done" button for website visit after user sends a message.
        set_user_state(user_id, 'done_website_check')
        keyboard = [[InlineKeyboardButton(get_translation(lang, "done_button"), callback_data='done_website')]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text(get_translation(lang, "after_visit_website_message"), reply_markup=reply_markup)
