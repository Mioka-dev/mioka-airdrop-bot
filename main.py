...
async def send_admin_data(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user.id == ADMIN_ID:
        ...
        # Prepare a long string to send as a text file
        output = io.StringIO()
        output.write("Telegram ID,Telegram Username,Twitter Username,Wallet Address,Referred By ID,Direct Referrals,Indirect Referrals,Total Tokens\n")
        ...
        for user_id, user_info in users_data.items():
            if 'wallet_address' in user_info:
                ...
                output.write(f"{user_id},"
                             ...
                             f"{user_info.get('twitter_username', 'N/A')},"
                             f"{user_info.get('wallet_address', 'N/A')},"
                             f"{referred_by.get(user_id, 'N/A')},"
                             f"{direct_count},"
                             f"{indirect_count},"
                             f"{total_tokens}\n")
        ...
        await update.message.reply_document(document=output.getvalue().encode('utf-8'), filename="participants.txt")
