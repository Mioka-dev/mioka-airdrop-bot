...
# Your Admin Telegram ID from environment variables
# This is a safer way to manage the admin ID
ADMIN_ID_STR = os.environ.get('ADMIN_ID')
try:
    ADMIN_ID = int(ADMIN_ID_STR) if ADMIN_ID_STR else None
except (ValueError, TypeError):
    logging.error("ADMIN_ID environment variable is not a valid integer!")
    ADMIN_ID = None

# Your Admin Telegram Username (for backwards compatibility, less reliable)
ADMIN_USERNAME = 'MiokaToken'.lstrip('@')
AIRDROP_LIMIT = 4000
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    if user.id == ADMIN_ID:
        # It's better to fetch the admin's language or default to a safe one
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id == ADMIN_ID:
        # Silently ignore messages from admin that are not commands
        return
...
async def send_admin_data(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    if user.id == ADMIN_ID:
        # Filter for users who have completed the airdrop
        data_list = []
        ...
        if not data_list:
            await update.callback_query.message.reply_text("There is no airdrop data to export yet.")
            return
        ...
        try:
            ...
            await update.callback_query.message.reply_document(open(file_path, 'rb'), caption="Airdrop participants data")
            os.remove(file_path)
        except ImportError:
            await update.callback_query.message.reply_text("The required libraries for Excel export ('pandas', 'openpyxl') are not installed. Please check the `requirements.txt` file and your Render settings.")
...
def main() -> None:
    ...
    if ADMIN_ID is None:
        logging.error("ADMIN_ID is not set or is invalid! Please set the 'ADMIN_ID' environment variable.")
        return
